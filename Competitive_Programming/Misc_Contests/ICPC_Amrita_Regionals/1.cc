...
// https://codedrills.io/contests/icpc-amritapuri-2020-regional-round/problems/the-invasion-of-balaji
void solve(){
    ...
    vi prmn = pre; 
    fr(i,1,n-1) prmn[i]=min(prmn[i],prmn[i-1]);

    vi sumx = pre;
    frr(i,n-2,0) sumx[i]=max(sumx[i+1],sumx[i]);

    // Answer is never negative since the empty army has 0 total usefullness.
    int ans = max(0ll,*max_element(all(u)));

    fr(i,0,n-1){

        int cur = max(0ll,q*t[i]);
        // Once we fix a index i to train, we need to pick left & right indices to form a subarray with maximum sums.
        // left_sum =  pre[i] - pre[j-1]  -> since pre[i] is fixed, we need to pick j such that `pre[j-1]` is minimum so that left sum is max.
        // right_sum =  pre[j] - pre[i]   -> since pre[i] is fixed, we need to pick j such that `pre[j]` is maximum so that right sum is max.
       
        int left_sum = pre[i] - min( 0ll,i ? prmn[i-1] : 0);

        int right_sum = sumx[i]-pre[i]; 
        
        ans = max( ans, left_sum + cur + right_sum );
    }

...
void solve() {
  int n,m ; cin >> n >> m;
  int u,v; vector<int> weaker(n+1), edges(n+1);  
  ...
  int q; cin >> q;
  while(q--){
      int type, newu, newv; cin >> type;
      if (type != 3){
          cin >> newu >> newv;
          if (type == 1){   // Add

              int usur=0, vsur=0;
              if (weaker[newu] == edges[newu]){
                  usur = 1;
              } 
              if (weaker[newv] == edges[newv]){
                  vsur = 1;
              } 

              edges[newu]++, edges[newv]++;
              if (newu < newv){ weaker[newv]++; }
              else { weaker[newu]++; } 

              if (!usur){  // Before it is not survived
                   if (weaker[newu] == edges[newu]){  // Now survived.
                       ans++;
                   }
              }
              else{  // Before it is survived
                  if (weaker[newu] != edges[newu]){ // Now it is not survived.
                       ans--;
                   }
              }

              if (!vsur){  // Before it is not survived
                   if (weaker[newv] == edges[newv]){  // Now survived.
                       ans++;
                   }
              }
              else{  // Before it is survived
                  if (weaker[newv] != edges[newv]){ // Now it is not survived.
                       ans--;
                   }
              }

...
void solve() {
  ...
  int W,H,x1,x2,y1,y2,w,h; cin >> W >> H >> x1 >> y1 >> x2 >> y2 >> w >> h;

  if ( max(x1, W-x2) >= w)  { cout << 0; return; }
  if ( max(y1, H-y2) >= h)  { cout << 0; return; }


   int notpossx = 0, notpossy = 0; 
   int xmove = 0, ymove = 0;
   if ( ( w - max(x1, W-x2) ) > min(x1, W-x2) ) {
        notpossx = 1; 
   }
   else{
       xmove = w - max(x1, W-x2);

...
void solve() {
  ...
  do {
    for (int i = 0; i < n; ++i)
      pr[cur][i + 1] = pr[cur][i] + (s[i] != t[i % 3]);  // It means that we are checking s & string of len = s.size() made of `t`.
    ++cur;
  ...
  while (m--) {
    ...
    cin >> l >> r;
    int ans = n;
    // Taking min. among all the possible sequences made of permutation of `abc`
    for (int i = 0; i < 6; ++i)

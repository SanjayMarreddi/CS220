...
void solve() {
  int m; cin >> m;
  vi r1(m), r2(m); int sum1=0,sum2=0;
  fr(i,0,m-1) { cin >> r1[i]; sum1 += r1[i]; }
  fr(i,0,m-1) { cin >> r2[i]; sum2 += r2[i]; } 

  vi r1s(m), r2p(m);
  r2p[0] = r2[0];
  fr(i,1,m-1) { r2p[i] = r2p[i-1] + r2[i]; }

  r1s[m-1] = r1[m-1];
  frr(i,m-2,0) { r1s[i] = r1s[i+1] + r1[i]; }

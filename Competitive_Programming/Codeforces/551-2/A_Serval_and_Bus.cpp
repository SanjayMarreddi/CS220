...
void solve() {
  int n,t; cin >> n >> t;
  vpi v(n); vpi greateq; vvi less;
  fr(i,0,n-1)
    { cin >> v[i].fi >> v[i].se; 
        if (v[i].fi >= t) { greateq.pb({ v[i].fi, i+1 }) ; }
        else { less.pb( { v[i].fi, v[i].se, i+1} ) ; }     }

  sort(all(greateq));
  vpi lessres;
  trav(e, less){
      int s = e[0], d = e[1], i = e[2];
      lessres.pb( { ( ( (t - s)  + (d-1) )/d )*d + s ,i } ); 
  } 

  sort(all(lessres));

  if (lessres.size() == 0){
      cout << greateq[0].se; return;
  }

   if (greateq.size() == 0){
      cout << lessres[0].se; return;
  }


  int one =  lessres[0].fi, two = greateq[0].fi;
  if (one >= two){
      cout << greateq[0].se;
  }
  else{

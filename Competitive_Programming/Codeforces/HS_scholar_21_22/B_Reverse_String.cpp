...
void solve() {
    string s,t; cin >> s >> t; int n = t.size();

    fr(i,0,n-1){

        string p1 = t.substr(0,i+1); string p1rev = p1; reverse(all(p1rev));
        string p2 = t.substr(i+1); string p2rev = p2; reverse(all(p2rev));
        dbg(p1,p2);
        
        auto itr1 = s.find(p1); auto itr2 = s.find(p2);
        auto itr1r = s.find(p1rev); auto itr2r = s.find(p2rev);

        if ( itr1 !=  string::npos ){
            if (itr2r != string::npos){
                if (itr2r + p2.size() < itr1 )
                    cout << "YES"; return;
            }
        }

        if ( itr2 !=  string::npos ){
            if (itr1r != string::npos){
                if (itr1r + p1.size() < itr2 )
                    cout << "YES"; return;
            }
        }

...
void solve() {
    ...
    map<int,int> cnt; map<int,vi> loc;
    int n,k; cin >> n >> k; vi a(n); 
    fr(i,0,n-1){  cin >> a[i]; cnt[a[i]]++; loc[a[i]].pb(i); }
    vi colors(n);

    map<int,int> colortracker;
    // coloring elements with freq >= k. 
    fr(i,0,n-1){
        if ( cnt[a[i]]  >= k and colortracker[a[i]] < k){
            colors[i] = colortracker[a[i]]+1;
            colortracker[a[i]]++;
        }
    }

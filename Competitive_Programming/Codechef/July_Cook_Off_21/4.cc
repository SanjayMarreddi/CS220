...
void solve() {
  int n; cin >> n; unordered_map<int,int> freq; int miny = INT_MAX; multiset<int> sorted;
  vi v(n); fr(i,0,n-1) { cin >> v[i]; freq[v[i]]++; miny = min(miny, v[i]);  sorted.insert(v[i]); }
  ...
  if (miny == 0LL){
      ...
      int Maxele =  2*(1e9);
      int end = min(2*miny, Maxele);
      int ele = *upper_bound(all(sorted), miny);  // At pos in sorted, > miny
      if (ele > end or ele == sorted.size()){
          cout << n - freq[miny];
      }
      else{
          cout << n;
